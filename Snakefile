...
rule hisat2_3n_mapping_genome_SE:
    ...
    threads: 28 # extract_unmap_bam_internal_SE（18线程） 和 dedup_mapping（18线程） 执行任务，extract_unmap_bam_internal_SE先执行完，下一个是此rule，可取18线程
    shell:
        """
...
rule hisat2_3n_sort:
    ...
    threads: 4 # hisat2_3n_mapping_genome_SE（36线程） 执行任务，下一个是此rule和extract_unmap_bam_internal_SE（18线程）,可取36-18=18线程
